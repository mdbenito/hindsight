...
Hindsight 0.7.0 is a major release that makes Hindsight faster, leaner, and more broadly deployable. Banks can now run on horizontally scaled Postgres clusters with true BM25 search, multilingual deployments get first-class language tuning and a polyglot CJK backend, the graph layer is roughly half the size it used to be and self-heals after deletes, the Control Plane is available in eight languages, and consolidation gets meaningfully better at maintaining a clean observation set.
