...
Which converter runs (markitdown, Docling, LlamaParse, etc.) is selected per deployment via the file-conversion processor settings — see [Configuration → File Processing](../configuration#file-processing).
